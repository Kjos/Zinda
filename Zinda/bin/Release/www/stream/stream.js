...
function receiveLoop(interlace) {
	var fs = framestamp[interlace];
	var url = "stream.php?sid=" + SESSION_ID + "&framestamp=" + fs;
	...
	xhr.onload = function(e) {
		if (200 == this.status) {
			...
			receiveLoop(interlace);
		}
	};
    xhr.onerror = function (e) {
		console.log('error', xhr, xhr.status, xhr.responseText);
		setTimeout(function() {receiveLoop(interlace);}, 1000);	
    };
...
function parseData(xhr, e) {
	...
	var fs = parseInt(xhr.getResponseHeader("framestamp"));
	var interlace = fs % 2;
	framestamp[interlace] = fs;
	receiveLoop(interlace);
